...
class ChatGPTThread(QThread):
    ...
    def run(self):
        ...
        self.parent.conversation_window.insertHtml('<br></p>')

        max_tokens = self.parent.max_tokens.value()
        temperature = float('{0:.2f}'.format(self.parent.temperature.value() / 100))
        try:
            while response is None:
                response = self.parent.chatgpt.send_request(prompt_string, max_tokens, temperature)
...
class AIAssistant(QWidget):
    ...
    def init_ui(self):
        ...
        self.max_tokens = QSlider(Qt.Orientation.Horizontal)
        self.max_tokens.setMinimum(10)
        self.max_tokens.setMaximum(4096)
        self.max_tokens.setSingleStep(500)
        self.max_tokens.setPageStep(500)
        self.max_tokens.setValue(200)
        self.max_tokens.setToolTip('Maximum tokens ChatGPT can consume')
        ...
        self.temperature.setToolTip('Randomness of the response')
...
class AppWindow(QWidget):
    def __init__(self):
        ...
        self.setWindowTitle('ChatGPT AI Assistant (By Timothy Poston) v1')
        self.setStyleSheet('''
            QWidget {
                font-size: 15px;
            }
